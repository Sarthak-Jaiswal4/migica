  ...
  return (
    <div className="w-full max-w-[1400px] mx-auto px-4 md:px-8 bg-gray-200 rounded-lg pt-8 pb-4">
      <div className="mb-8 flex flex-row md:items-end justify-between gap-4">
        <div>
          <h2 className="text-3xl md:text-4xl tracking-normal font-light text-foreground font-[style]">{getDescriptiveHeading(name)}</h2>
          <p className="text-sm font-medium text-muted-foreground mt-1">{products.length} items</p>
        </div>
        <Button
          variant="outline"
          className="self-start md:self-auto group rounded-full px-6 h-10 text-sm font-semibold border-neutral-300 hover:bg-neutral-100 hover:text-foreground transition-all active:scale-95 hover:cursor-pointer"
          asChild
        >
      ...
      {inlineQuote ? (
        <aside className="mb-8 max-w-3xl" aria-label="Customer quote">
          <div className="border-l border-neutral-900/20 pl-5 sm:pl-6">
            ...
            <footer className="mt-3 text-[14px] font-medium tracking-tight text-neutral tracking-wide text-muted-foreground">
